// File: src/pages/Login.jsx (slightly updated)
import React from 'react';
import '../styles/login.css';

export default function LoginPage({ onLogin, openForm = false, onBack }) {
  const [showForm, setShowForm] = React.useState(!!openForm);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Ajusta el endpoint a tu backend FastAPI si hace falta
  ...
  const handleVolver = () => {
    // If parent provided onBack, call it (to go back to landing). Otherwise just toggle the internal intro.
          ...
          {!showForm ? (
            <div className="auth-intro">
              ...
              <p className="auth-desc">
                "Plataforma de monitoreo y alerta temprana ... (resumen)"
              </p>
            ...
            <form className="auth-form" onSubmit={handleSubmit} aria-label="Formulario de inicio de sesión">
              ...
              <div className="form-actions">
                ...
                <button type="button" className="btn btn-ghost" onClick={handleVolver}>Volver</button>
              </div>
